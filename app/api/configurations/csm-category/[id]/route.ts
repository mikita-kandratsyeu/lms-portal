...
export const PATCH = async (req: NextRequest, props: { params: Promise<{ id: string }> }) => {
  ...
  try {
    ...
    const category = await db.csmCategory.update({
      where: { id },
      data: values,
    });
    ...
    console.error('[CSM_CATEGORY_PATCH]', error);
...
export const DELETE = async (_: NextRequest, props: { params: Promise<{ id: string }> }) => {
  ...
  try {
    ...
    const category = await db.csmCategory.delete({
      where: { id },
    });

    return NextResponse.json(category);
  } catch (error) {
    console.error('[CSM_CATEGORY_DELETE]', error);
