...
export const CategoryDialog = ({ isOpen, onClose, editingItem }: CategoryDialogProps) => {
  const t = useTranslations('owner.configurations.categories.dialog');
  const tMessages = useTranslations('owner.configurations.categories.messages');
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      if (editingItem) {
        await fetcher.patch(`/api/configurations/category/${editingItem.id}`, {
          body: values,
          responseType: 'json',
        });
        toast({ title: tMessages('updateSuccess'), type: 'success' });
      } else {
        await fetcher.post('/api/configurations/category', {
          body: values,
          responseType: 'json',
        });
